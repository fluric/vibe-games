...
class ReversiNet(nn.Module, BaseNet):
    """
    AlphaZero-style dual-head network for Reversi.
    Action space = 65 (64 positions + 1 pass action).
    Input = 3 planes of 8x8.
    """
...
def create_net(device: torch.device | None = None) -> ReversiNet:
    if device is None:
        device = get_device()
    net = ReversiNet()
    net.to(device)
...
def save_checkpoint(net: ReversiNet, path: str) -> None:
    torch.save(net.state_dict(), path)


def load_checkpoint(path: str, device: torch.device | None = None) -> ReversiNet:
    if device is None:
        device = get_device()
    net = ReversiNet()
