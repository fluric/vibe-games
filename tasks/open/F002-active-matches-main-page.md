...
- **2026-06-30**: Implemented `OngoingMatchesPanel` to show in-progress matches globally on the `LobbyPage`. Also updated the `GET /games` backend endpoint to support filtering by status (waiting, in_progress). Moved back to `open/` because the testing Acceptance Criteria are not yet completed!
