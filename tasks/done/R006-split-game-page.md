...
## Acceptance Criteria
- [x] Create `frontend/src/components/game/` directory.
- [x] Extract generic layout and player stats to `GameLayout.tsx` and `PlayerStats.tsx`.
- [x] Extract the action panels and turn controls into `GameControls.tsx`.
- [x] Ensure `GamePage.tsx` drops under 300 lines by delegating these rendering concerns.
- [x] The Vite build (`npm run test:quick`) still passes successfully.
