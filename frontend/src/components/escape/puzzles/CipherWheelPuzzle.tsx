  ...
  return (
    <div className="escape-puzzle escape-cipher">
      ...
      <p className="cipher-instruction" style={{ fontSize: '0.85rem', color: '#9ca3af', margin: '0.5rem 0 1.5rem', textAlign: 'center', maxWidth: '400px', alignSelf: 'center' }}>
        <strong>{t('ui_hint', { defaultValue: 'Hint:' })}</strong> {t('ui_cipher_instruction', { defaultValue: 'This is a Caesar cipher. By dragging the wheel, you shift every letter in the encoded message by a fixed number of spaces across the alphabet.' })}
      </p>

      {/* Ciphertext display */}
      <div className="cipher-scroll">
        <span className="cipher-label">{t('ui_encoded_message', { defaultValue: 'Encoded message:' })}</span>
        <span className="cipher-ciphertext">{config.ciphertext}</span>
        <span className="cipher-arrow">↓ {t('ui_shift', { defaultValue: 'shift' })} = {shift}</span>
      ...
      <div
        ...
        aria-valuemax={25}
        aria-valuenow={shift}
        aria-label={t('ui_cipher_wheel_aria', { defaultValue: 'Cipher shift wheel — drag left or right' })}
        tabIndex={0}
      >
        ...
        <div className="cipher-wheel-center">
          <span>{t('ui_drag', { defaultValue: 'DRAG' })}</span>
        </div>
      </div>
      ...
      >
        {solved ? t('ui_decoded', { defaultValue: '✓ Decoded!' }) : locked ? t('ui_system_locked', { defaultValue: 'System Locked...' }) : t('ui_confirm_decoding', { defaultValue: 'Confirm Decoding' })}
      </button>
    </div>
  );
}
